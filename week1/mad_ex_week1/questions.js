...
function capitalizeSentence(sentence) {
  ...
  words.forEach(word => {
    ...
    let new_word = first_char.toUpperCase() + word.slice(1)
...
function animalClassify(type, weight) {
  ...
  return {type: type, size: final_size}
}
...
function describePerson(person) {
  ...
  let greeting = person.name + " is a " + person.age + "-year-old " 
  if (person.gender == 'm'){
    greeting += 'man. He '
  } else {
    greeting += 'woman. She '
  ...
  if (person.hobby){
    greeting += "likes " + person.hobby + "!"
  } else {
    greeting += "has no hobbies."
  }
    return greeting
}
...
function analyzeMarks(marks) {
  // Your implementation here
  let marks_array = Object.values(marks)
  if (marks_array.length == 0){
    return [0,0,0]
  }
  let total_sum = 0
  marks_array.forEach(mark => {
    total_sum += mark
  })
  let roundedAverage = parseFloat((total_sum / marks_array.length).toFixed(2)); 

  return [Math.min(...marks_array), Math.max(...marks_array), roundedAverage]
}
